...
public class ProcessService extends BasicService<Object> {
	...
	public void luuDuLieuDuAnVaBatDauXucTien(Execution execution) {
		...
		if (model.getGiaoViec().getTaiLieu().getNameHash() == null) {
			model.getGiaoViec().setTaiLieu(null);
		} else {
	...
	public void taoGiaiDoanDuAn(DuAn model) {
		GiaiDoanDuAn giaiDoanDuAn = new GiaiDoanDuAn();
		giaiDoanDuAn.setDuAn(model);
		giaiDoanDuAn.setGiaiDoanXucTien(GiaiDoanXucTien.GIAI_DOAN_MOT);
		giaiDoanDuAn.saveNotShowNotification();
	}
	...
	public void luuTaiLieuDuAnAndCheck(DuAn model) {
		if (model.getTaiLieuNDT() == null) {
			return;
		}
		if (model.getTaiLieuNDT().getNameHash() == null) {
			model.setTaiLieuNDT(null);
		} else {
	...
	public void validateDuLieuGiaiDoanNamVaKetThucDuAn(Execution execution) {
		...
		if (model.getTenDuAn() == null || model.getTenDuAn().isEmpty() || model.getLinhVuc() == null
				...
				|| model.getQuyMoDuAn().isEmpty() || model.getTongVonDauTu() == null 
				|| model.getMucTieuDuAn() == null || model.getMucTieuDuAn().isEmpty()
				|| model.getDienTichSuDungDat() == null 
		...
		if (model.getTongVonDauTu() != null) {
			if (model.getValidateThongTinDuAn(model.getTongVonDauTu(), "Tổng vốn đầu tư", model.getGiaiDoanDuAn().getGiaiDoanXucTien())) {
				return;
			}
		}
		if (model.getDienTichSuDungDat() != null) {
			if (model.getValidateThongTinDuAn(model.getDienTichSuDungDat(), "Diện tích sử dụng đất", model.getGiaiDoanDuAn().getGiaiDoanXucTien())) {
				return;
			}
		}
		boolean result = kiemTraCongViecHoanThanh(duAn);
	...
	public void luuDuLieuDonVi(GiaiDoanDuAn giaiDoanDuAn, boolean luuLichSu) {
		giaiDoanDuAn.getDonViDuAn().forEach(item -> {
			...
			item.getDonVi().saveNotShowNotification();
	...
	public void thongBao(DuAn duAn, LoaiThongBao loaiThongBao, NhanVien nguoiNhan, NhanVien nguoiGui, String tenNguoiCu, boolean goRedirect) {
		...
		if (LoaiThongBao.CONG_VIEC_MOI.equals(loaiThongBao)) {
			thongBao.setNoiDung(nguoiNhan.getHoVaTen() + "@ có công việc mới @" + duAn.getGiaoViec().getTenCongViec() + "@ của dự án @" + duAn.getTenDuAn());
		}
		if (LoaiThongBao.CHUYEN_NGUOI_PHU_TRACH.equals(loaiThongBao)) {
			thongBao.setNoiDung("Dự án @" + duAn.getTenDuAn() + "@ mà bạn đang phụ trách được chuyển cho @" + tenNguoiCu + "@ phụ trách.");
		}
